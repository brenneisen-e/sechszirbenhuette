    ...
    if (body.images && body.images.length > 0) {
      for (let i = 0; i < body.images.length; i++) {
        ...
        await env.DB.prepare(
          ...
          .bind(postId, img.image_url || '', img.image_alt || null, img.caption || null, i)
          .run();
      }
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Get current post
    const currentPost = await env.DB.prepare('SELECT * FROM blog_posts WHERE id = ?')
      .bind(body.id)
      .first<BlogPost>();
    ...
    if (body.slug && body.slug !== currentPost.slug) {
      const existing = await env.DB.prepare('SELECT id FROM blog_posts WHERE slug = ? AND id != ?')
    ...
    if (body.images !== undefined) {
      ...
      // Add new images (also slides without image_url – keeps the tile count intact)
      for (let i = 0; i < body.images.length; i++) {
        const img = body.images[i];
        if (!img) continue;
        await env.DB.prepare(
          ...
          .bind(body.id, img.image_url || '', img.image_alt || null, img.caption || null, i)
          .run();
      }
    }
