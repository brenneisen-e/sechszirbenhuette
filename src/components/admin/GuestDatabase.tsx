      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-gray-900">Setup & Konfiguration</span>
            {setupStatus && (
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                setupStatus.guestsTableExists && setupStatus.guestsCount > 0
                  ? 'bg-green-100 text-green-700'
                  : 'bg-amber-100 text-amber-700'
              }`}>
                {setupStatus.guestsTableExists && setupStatus.guestsCount > 0 ? 'Bereit' : 'Setup erforderlich'}
              </span>
            )}
          </div>
        ...
        {showSetup && (
          <div className="px-4 pb-4 border-t">
            ...
            <div className="mt-4 grid md:grid-cols-4 gap-4">
              <div className={`p-3 rounded-lg ${setupStatus?.guestsTableExists ? 'bg-green-50' : 'bg-gray-50'}`}>
                <div className="flex items-center gap-2 mb-1">
                  {setupStatus?.guestsTableExists ? (
                    <CheckCircle2 className="w-4 h-4 text-green-600" />
                  ) : (
                    <Circle className="w-4 h-4 text-gray-400" />
                  )}
                  <span className="text-sm font-medium">Gäste-Tabelle</span>
                </div>
                <p className="text-xs text-gray-600">
                  {setupStatus?.guestsTableExists ? `${setupStatus.guestsCount} Einträge` : 'Nicht vorhanden'}
                </p>
              </div>

              <div className={`p-3 rounded-lg ${setupStatus?.emailsTableExists ? 'bg-green-50' : 'bg-gray-50'}`}>
                <div className="flex items-center gap-2 mb-1">
                  {setupStatus?.emailsTableExists ? (
                    <CheckCircle2 className="w-4 h-4 text-green-600" />
                  ) : (
                    <Circle className="w-4 h-4 text-gray-400" />
                  )}
                  <span className="text-sm font-medium">E-Mail-Tabelle</span>
                </div>
                <p className="text-xs text-gray-600">
                  {setupStatus?.emailsTableExists ? `${setupStatus.emailsCount} E-Mails` : 'Nicht vorhanden'}
                </p>
              </div>
              ...
              <div className="p-3 rounded-lg bg-gray-50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium">Gäste-Import</span>
                </div>
                <p className="text-xs text-gray-600">
                  {setupStatus && setupStatus.guestsCount > 0 ? 'Daten vorhanden' : 'Import ausstehend'}
                </p>
              </div>
              ...
              <div className="p-3 rounded-lg bg-gray-50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium">E-Mail-Sync</span>
                </div>
                <p className="text-xs text-gray-600">
                  {setupStatus && setupStatus.emailsCount > 0 ? 'E-Mails synchronisiert' : 'Terminal-Befehl nötig'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">Datenbank-Tabellen erstellen</p>
                    <p className="text-xs text-gray-500">Erstellt die Tabellen für Gäste und E-Mails</p>
                  </div>
                </div>
                <button
                  onClick={() => runSetupAction('migrate_guests_tables')}
                  disabled={isRunningSetup !== null}
                ...
                >
                  {isRunningSetup === 'migrate_guests_tables' ? (
                    <>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">Gästedaten importieren</p>
                    <p className="text-xs text-gray-500">Importiert die historischen Buchungsdaten</p>
                  </div>
                </div>
                <button
                  onClick={() => runSetupAction('import_guests')}
                  disabled={isRunningSetup !== null || !setupStatus?.guestsTableExists}
                  className={`px-3 py-1.5 text-sm rounded-lg flex items-center gap-2 ${
                    setupStatus && setupStatus.guestsCount > 0
                      ? 'bg-green-100 text-green-700'
                      : setupStatus?.guestsTableExists
                        ? 'bg-blue-600 text-white hover:bg-blue-700'
              ...
              <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-sm">FeWo-Buchungen importieren</p>
                      <p className="text-xs text-gray-500">
                        {fewoPreview ? `${fewoPreview.totalBookings} Buchungen mit Nebenkosten & Kommunikation` : 'Lädt...'}
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={importFewoBookings}
                    disabled={isImportingFewo || isRunningSetup !== null}
                  ...
                  <div className="mt-4 p-3 bg-white rounded-lg">
                    <div className="grid grid-cols-4 gap-2 mb-3">
                      <div className="text-center p-2 bg-green-50 rounded">
                        <p className="text-lg font-bold text-green-600">{fewoImportResult.updated}</p>
                        <p className="text-xs text-gray-500">Aktualisiert</p>
                      </div>
                      <div className="text-center p-2 bg-blue-50 rounded">
                        <p className="text-lg font-bold text-blue-600">{fewoImportResult.created}</p>
                        <p className="text-xs text-gray-500">Erstellt</p>
                      </div>
                      <div className="text-center p-2 bg-amber-50 rounded">
                        <p className="text-lg font-bold text-amber-600">{fewoImportResult.notFound}</p>
                        <p className="text-xs text-gray-500">Nicht gefunden</p>
                      </div>
                      <div className="text-center p-2 bg-red-50 rounded">
                        <p className="text-lg font-bold text-red-600">{fewoImportResult.errors}</p>
                        <p className="text-xs text-gray-500">Fehler</p>
                      </div>
                    </div>
                    <div className="max-h-32 overflow-y-auto space-y-1">
                      {fewoImportResult.results.map((r, i) => (
              ...
              <div className="flex items-center justify-between p-3 bg-red-50 border border-red-200 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm text-red-800">Alle Daten löschen</p>
                    <p className="text-xs text-red-600">Löscht alle Gäste und E-Mails unwiderruflich</p>
                  </div>
                </div>
                <button
                  onClick={() => {
                    if (confirm('Wirklich ALLE Gäste und E-Mails löschen? Diese Aktion kann nicht rückgängig gemacht werden!')) {
                      runSetupAction('delete_all_data');
                    }
                  }}
                  disabled={isRunningSetup !== null || (!setupStatus?.guestsCount && !setupStatus?.emailsCount)}
                  className="px-3 py-1.5 text-sm rounded-lg flex items-center gap-2 bg-red-600 text-white hover:bg-red-700 disabled:opacity-50"
                >
                  {isRunningSetup === 'delete_all_data' ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      Löscht...
                    </>
              ...
              {/* Info about IMAP sync */}
              {syncResults.length > 0 && syncResults.some(r => r.includes('Verbunden') || r.includes('Login')) && (
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-xs text-green-700">
                    <strong>Erfolg:</strong> E-Mail-Synchronisation direkt über Cloudflare Workers TCP-Sockets.
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Results Log */}
            {(setupResults.length > 0 || syncResults.length > 0) && (
              <div className="mt-4 p-3 bg-gray-900 rounded-lg">
                <p className="text-xs text-gray-400 mb-2">Ausführungs-Log:</p>
                <div className="space-y-1">
                  {[...setupResults, ...syncResults].map((result, idx) => (
