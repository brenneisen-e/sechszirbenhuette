...
import type {
  ...
  GuestResponse,
  BookingsResponse,
} from './guest-database/types';
import { STANDARD_TASKS } from './guest-database/constants';
import { AdminCalendar } from './guest-database/AdminCalendar';
import { GuestEditModal } from './guest-database/GuestEditModal';
import { CreateGuestModal } from './guest-database/CreateGuestModal';
import { AllTasksView } from './guest-database/AllTasksView';
import { GuestTableRow } from './guest-database/GuestTableRow';
import BookingCard from './guest-database/BookingCard';
import {
  sortGuests,
  getEffectiveStatus,
  getEffectiveBookingStatus,
} from './guest-database/helpers';
...
export default function GuestDatabase({ onDataLoaded, demoMode = false }: GuestDatabaseProps) {
  ...
  const getLatestBooking = useCallback(
    (guest: Guest): { arrival: string | null; departure: string | null } => {
  ...
  const getGuestEffectiveStatus = useCallback(
    (guest: Guest): string => {
      const bookings = guestBookings[guest.id] || [];
  ...
  const saveBooking = async (
    ...
    try {
      ...
      const response = await fetch('/api/admin/bookings', {
        ...
        body: JSON.stringify(
          isNewBooking ? { ...bookingData, guest_id: addingBookingForGuest } : bookingData,
        ),
      });
      ...
      if (data.error) {
        ...
        if (uploadedFile && guestId) {
          try {
            ...
            formData.append(
              'description',
              `Buchung #${data.booking.booking_number || data.booking.id} - Importiert am ${new Date().toLocaleDateString('de-DE')}`,
            );
            ...
            if (docResult.success && docResult.r2_key) {
              const additionalCosts = data.booking.additional_costs
                ? JSON.parse(data.booking.additional_costs)
                : {};
      ...
      if (data.error) {
        ...
        setGuestBookings((prev) => {
          ...
          for (const guestId of Object.keys(updated)) {
            ...
            const idx = bookings.findIndex((b) => b.id === bookingId);
            if (idx >= 0) {
              updated[Number(guestId)] = [...bookings.slice(0, idx), ...bookings.slice(idx + 1)];
              break;
            }
          }
          return updated;
  ...
  const updateBookingPaymentStatus = async (
    ...
    try {
      ...
      if (data.error) {
        ...
        setSuccess(
          field === 'deposit_paid' ? 'Anzahlung aktualisiert' : 'Restzahlung aktualisiert',
        );
      }
    } catch (err) {
  ...
  const updateBookingTransactions = async (
    ...
    try {
      ...
      const booking = Object.values(guestBookings)
        .flat()
  ...
  const toggleTaskStatus = async (taskId: number, isCompleted: boolean) => {
    try {
      ...
      if (data.error) {
        ...
        // Update in all tasks
        setAllTasks((prev) => prev.map((t) => (t.id === taskId ? data.task! : t)));
        // Also update in guest tasks if loaded
        setGuestTasks((prev) => prev.map((t) => (t.id === taskId ? data.task! : t)));
      }
    } catch (err) {
  ...
  const updateGuestStatus = async (guestId: number, newStatus: string) => {
    try {
      ...
      const bookingsToUpdate = bookings.filter(
        (b) => b.status !== 'cancelled' && b.status !== newStatus,
      );
      await Promise.all(
  ...
  const togglePayment = async (
    ...
    try {
      ...
      if (data.error) {
        ...
        // Update local state
        setGuests(
          guests.map((g) => (g.id === guestId ? { ...g, [updateField]: isPaid ? 1 : 0 } : g)),
        );

        // Also update the corresponding task if it exists
        const correspondingTask = guestTasks.find(
          (t) => t.guest_id === guestId && t.title === taskTitle,
        );
        if (correspondingTask) {
          ...
          if (taskData.task) {
            setGuestTasks(
              guestTasks.map((t) => (t.id === correspondingTask.id ? taskData.task! : t)),
            );
          }
        }
  ...
  const handleCreateGuest = async (
        ...
        if (bookingData.error) {
          ...
          if (importFile && selectedExistingGuestId) {
            try {
              ...
              formData.append(
                'description',
                `Importiert am ${new Date().toLocaleDateString('de-DE')}`,
              );
    ...
    try {
      const guestToCreate = {
        ...newGuest,
        year: newGuest.arrival_date
          ? new Date(newGuest.arrival_date).getFullYear()
          : new Date().getFullYear(),
        month: newGuest.arrival_date
          ? new Date(newGuest.arrival_date).toLocaleString('de-DE', { month: 'long' })
          : null,
      ...
      if (data.error) {
        ...
        if (data.guest && createStandardTasks) {
          ...
          for (const st of STANDARD_TASKS) {
            try {
              ...
              if (!taskResponse.ok) {
                ...
                const shouldSetNA =
                  (isBookingCom && bookingComNATasks.includes(st.key)) ||
          ...
          if (taskErrors > 0) {
            setError(
              `${taskErrors} Aufgabe(n) konnten nicht erstellt werden. Bitte führe "Datenbank-Tabellen erstellen" im Setup aus.`,
            );
          }
      ...
      if (data.error) {
        ...
        if (task) {
          ...
          if (guestField) {
            ...
            // Update local state
            setGuests(
              guests.map((g) => (g.id === task.guest_id ? { ...g, [guestField]: fieldValue } : g)),
            );
          }
        }
      }
  ...
  const years = [...new Set(guests.map((g) => g.year).filter(Boolean))].sort(
    (a, b) => (b || 0) - (a || 0),
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="hidden sm:block">
          <h2 className="text-lg font-medium text-gray-900">Gästedatenbank</h2>
          <p className="text-sm text-gray-500">{guests.length} Gäste gesamt</p>
        </div>
              ...
              {loading ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm mt-2">
                    Öffne Setup oben und klicke auf &quot;Gäste importieren&quot;
                  </p>
                </div>
              ) : (
                      ...
                      {displayedGuests.map((guest) => {
                        ...
                        return (
                          <GuestTableRow
                            ...
                            onToggleCleaningCash={(bookingId, isCash) =>
                              updateBookingCleaningCash(bookingId, isCash)
                            }
                            onTogglePaymentStatus={(bookingId, field, value) =>
                              updateBookingPaymentStatus(bookingId, field, value)
                            }
                            onUpdateTransactions={(bookingId, transactions, payoutDate) =>
                              updateBookingTransactions(bookingId, transactions, payoutDate)
                            }
                            onUploadDocument={async (bookingId, guestId, file) => {
                              ...
                              formData.append(
                                'documentType',
                                file.type.includes('pdf') ? 'booking_pdf' : 'screenshot',
                              );
                              formData.append(
                                'description',
                                `Buchung #${bookingId} - ${new Date().toLocaleDateString('de-DE')}`,
                              );
                            ...
                            onUpdateNote={(noteId, content) =>
                              updateNote(guest.id, noteId, content)
                            }
                            ...
                            onUpdateTask={async (taskId, updates) => {
                              ...
                              if (data.error) {
                                ...
                                setGuestTasks(
                                  guestTasks.map((t) => (t.id === taskId ? data.task! : t)),
                                );
                              }
                            }}
                        ...
                        {hideCompleted ? (
                          <>
                            ...
                            <span className="text-sm">
                              {completedCount} abgeschlossene Gäste anzeigen
                            </span>
                          </>
                        ) : (
                          <>
