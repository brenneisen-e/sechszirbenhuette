...
function parseGermanDate(dateStr: string): string | null {
  ...
  const match = dateStr.match(
    /^(\d{1,2})\.\s*([A-Za-zäöü]+)\.?\s+(\d{4})(?:\s*[-–—]\s*(\d{1,2}:\d{2}))?/i,
  );
...
function parseCommunication(
...
): Array<{ sender: 'host' | 'guest'; text: string; date?: string }> {
  ...
  for (const line of lines) {
    ...
    const inlineDateMatch = trimmedLine.match(
    ...
    if (!currentSender) {
      ...
      if (/^Sehr geehrte/i.test(trimmedLine) || /^Liebe(?:r|s)?\s/i.test(trimmedLine)) {
        ...
        /^Vielen Dank für Ihr/i.test(trimmedLine) ||
        /^herzlich willkommen/i.test(trimmedLine)
      ) {
        currentSender = 'host';
          ...
          <div className="max-h-96 overflow-y-auto space-y-3 p-2">
            {hasFewoMessages ? (
              ...
              legacyMessages.map((msg, idx) => (
                ...
                >
                  {msg.date && <div className="text-xs text-gray-400 mb-1 px-1">{msg.date}</div>}
                  <div
                    className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${
                      msg.sender === 'host'
                        ? 'bg-blue-600 text-white rounded-br-none'
...
export function BookingDetail({
  ...
  const isFeWo =
    booking.platform?.toLowerCase() === 'fewo' ||
  ...
  let communication = '';
  let document: { r2_key: string; filename: string; type: string; uploaded_at?: string } | null =
    null;
  ...
  if (
    !platformFees.nebenkosten_income &&
    (platformFees.guest_total_payment || 0) > 0 &&
    (platformFees.platform_service_fee || 0) > 0
  ) {
    const calculatedNK =
      (platformFees.guest_total_payment || 0) -
      (booking.rental_price || 0) -
      cleaningCostForFallback -
      (platformFees.platform_service_fee || 0);
    if (calculatedNK > 0) {
      platformFees = {
        ...platformFees,
        nebenkosten_income: calculatedNK,
        cleaning_fee_income: cleaningCostForFallback,
      };
    }
  }
    ...
    <div className="space-y-4">
          ...
          {onDelete && (
            <button
              onClick={() => {
                if (
                  confirm(
                    'Buchung wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.',
                  )
                ) {
                  onDelete();
                }
      ...
      {!isPrivate && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div
            className={`${gesamtertrag >= 0 ? 'bg-emerald-50' : 'bg-red-50'} rounded-lg p-3 text-center`}
          >
            <div className="text-xs text-gray-500 mb-0.5">Kalk. Ertrag</div>
            <div
              className={`text-lg font-bold ${gesamtertrag >= 0 ? 'text-emerald-700' : 'text-red-600'}`}
            >
            ...
            <div className="space-y-2 text-sm">
              ...
              {isPlatformWithIncludedCosts && basisMiete === 0 ? (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-green-700">
                    {formatCurrency(gesamtauszahlung)}
                  </span>
                </div>
              ) : (
                <div className="flex justify-between">
              ...
              {isUtilitiesCash && calculatedCosts + kurtaxe > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-green-700">
                    {formatCurrency(calculatedCosts + kurtaxe)}
                  </span>
                </div>
              )}
              ...
              <div className="flex justify-between pt-1 border-t border-gray-200">
                ...
                <span className="font-bold text-green-700">{formatCurrency(totalEinzahlung)}</span>
              </div>

              {/* === KOSTEN === */}
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide pt-3">
                Kosten
              </div>
              <div>
                <button
                ...
                >
                  <span className="text-gray-600 flex items-center gap-1">
                    ...
                    {isUtilitiesCash ? ' (bar)' : isKurtaxeCash ? ' (Kurtaxe bar)' : ''}:
                    {showCostDetails ? (
                      <ChevronUp className="w-3 h-3" />
                    ) : (
                      <ChevronDown className="w-3 h-3" />
                    )}
                    {utilityCosts && !showCostDetails && (
                      <span className="text-xs text-gray-400 ml-1">{utilityCosts.details}</span>
                    )}
                  </span>
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">
                  Provision (10% v. {formatCurrency(mieterlos)}):
                </span>
        ...
        <div className="bg-gray-50 rounded-lg p-4 space-y-3">
          <h5 className="font-medium text-gray-900 flex items-center justify-between">
            ...
            {onUpdateTransactions &&
              ...
              (transactions.length > 0 || bankPayments.length === 0) && (
                <button
                  onClick={() => {
              ...
              <>
                {editedTransactions.map((t, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex items-center gap-2 py-1.5 text-sm bg-white rounded px-2 border border-gray-200"
                    >
                      <select
                        value={t.type}
                        onChange={(e) => {
                          ...
                          newTransactions[idx] = {
                            ...t,
                            type: e.target.value as 'payment' | 'refund',
                          };
                      ...
                      <input
                        ...
                        value={
                          t.description ||
                          `Zahlung ${idx + 1} von ${editedTransactions.filter((tx) => tx.type === 'payment').length}`
                        }
                        onChange={(e) => {
                          const newTransactions = [...editedTransactions];
              ...
              <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={isRefund ? 'text-orange-600' : 'text-gray-700'}>
                          {t.description ||
                            (isRefund
                              ? 'Erstattung'
                              : `Zahlung ${idx + 1} von ${transactions.filter((tx) => tx.type === 'payment').length}`)}
                        </span>
                        ...
                        {t.fee !== undefined && t.fee !== 0 && (
                          <span
                            className={`text-xs ${t.fee > 0 ? 'text-red-400' : 'text-green-400'}`}
                          >
                            (Gebühr: {t.fee > 0 ? '-' : '+'}
                            {formatCurrency(Math.abs(t.fee))})
                          </span>
                        )}
                      </div>
                      <span
                        className={`font-medium ${isRefund ? 'text-orange-600' : t.status === 'paid' ? 'text-green-600' : 'text-gray-600'}`}
                      >
                        {isRefund ? '-' : '+'}
                        {formatCurrency(t.amount)}
                        {t.status === 'paid' && !isRefund && (
                ...
                {(() => {
                  ...
                  const processingFee =
                    transactionFees > 0
                      ? transactionFees
                      : platformFees.payment_processing_fee || 0;
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                  ...
                  {(() => {
                    ...
                    return (
                      <span
                        className={`font-bold ${netPaid > 0 ? 'text-green-600' : 'text-gray-900'}`}
                      >
                    ...
                    return (
                      <div className="flex items-center justify-between pt-2 border-t border-gray-200 bg-emerald-50 -mx-3 px-3 py-2 rounded-b">
                        <span className="font-medium text-emerald-800">
                          {isFuture ? 'Auszahlung am:' : 'Eingegangen am:'}
                        </span>
              ...
              <>
                ...
                {bankPayments.map((payment, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-green-600" />
                      <span className="text-gray-700">{payment.notes || `Zahlung ${idx + 1}`}</span>
                      <span className="text-xs text-gray-400">
